        ...
        match k {
            _ if k > 0 => {
                ...
                for index in 0..code.len() {
                    ret[index] = tmp;
                    tmp = tmp - code[(index + 1) % code.len()]
                        + code[(index + 1 + (k as usize)) % code.len()];
                }
            }
            _ if k < 0 => {
                let mut tmp = code[(code.len() as i32 + k) as usize % code.len()..]
                    .iter()
                    .sum();

                for index in 0..code.len() {
                    ret[index] = tmp;
                    tmp = tmp - code[((code.len() + index) as i32 + k) as usize % code.len()]
                        + code[(code.len() + index) % code.len()];
                }
            }
            _ => {}
        }
